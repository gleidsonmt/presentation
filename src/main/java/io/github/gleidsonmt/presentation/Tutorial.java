...
//        super(context);
        ...
        btnMap.getStyleClass().add("btn-map");
//        btnTop.setVisible(false);

        root.widthProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue.doubleValue() < _break) minLayout();
            else maxLayout();
        });
...
//        StackPane.setMargin(btnMap, new Insets(10, 0, 0, 0));
    ...
    public void setBreak(double _break) {
        this._break = _break;
    }
...
//                c.setText(item.getIndex() + "." + count++);
        ...
        for (TreeTitle child : children) {
            ...
            VBox.setMargin(subMenu, new Insets(0, 0, 0, space += 5));
        }
...
//                label.setPosition(Scroll.getY(scroll, label)));
        ...
        dot.setFill(Color.gray(0.3, 0.5));
        dot.setStyle("-fx-fill: -text-color");
        dot.setRadius(2);
    ...
    public Tutorial build() {
        ...
        title.getStyleClass().addAll("bold", "h4");
...
//            Context context = (Context) System.getProperties().get("context");
////            context.flow().openAbsolute(aside, Pos.CENTER_RIGHT, Insets.EMPTY);
//            context.wrapper().show();
//            context.flow().slideInFromRight(aside);
//            context.wrapper().setOnClick(ev -> {
//                context.wrapper().close();
//                context.flow().slideOutFromRight(aside);
//            });
        });
